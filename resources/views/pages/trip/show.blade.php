...
<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    ...
    <div class="flex gap-2">
        <x-button as="a" href="{{ route('trips.results', ['from' => $trip->route->startCity->name, 'to' => $trip->route->arrivalCity->name, 'date' => $trip->date]) }}" variant="secondary">
        ...
        <section class="lg:col-span-7">
            <x-card>
                <div class="flex items-start justify-between gap-3 mb-6">
                    <div>
                        <div class="text-sm font-semibold">Taxi • {{ $trip->taxi->licence_plate }}</div>
                        <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Cliquez sur un siège pour le sélectionner.</p>
                    </div>
                    ...
                    <div class="max-w-[300px] mx-auto">
                        ...
                        <div class="grid gap-8">
                            <div class="grid grid-cols-2 gap-10">
                                ...
                                @include('partials.seat-item', ['seat' => $trip->taxi->seats->where('seat_number', 1)->first()])
                            </div>

                            <div class="grid grid-cols-3 gap-4">
                                @foreach($trip->taxi->seats->where('seat_number', '>', 1)->sortBy('seat_number') as $seat)
                                    @include('partials.seat-item', ['seat' => $seat])
                                @endforeach
                            </div>
                        </div>
                    </div>
            ...
            <div class="sticky top-24 space-y-4">
                ...
                <x-card>
                    ...
                    <div class="grid gap-4">
                        ...
                        <x-button type="submit" class="w-full" x-bind:disabled="selectedSeats.length === 0">
